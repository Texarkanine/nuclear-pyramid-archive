...
2. **Transform**: Copies downloaded files from `archive.org/` to `docs/site/` (except `From Gravitons to Galaxies.docx`, which stays on [GitHub Releases](https://github.com/Texarkanine/nuclear-pyramid-archive/releases) only), applying HTML transforms to `.php`/`.html` files: rewrites `http://nuclearpyramid.com` links to `https://`, rewrites `charset=iso-8859-1` to `charset=utf-8`, rewrites the book docx download link to that release asset (bypassing the nginx proxy for the 13MB file), and injects an "About" navigation link. Custom pages from `src/` are copied on top.
